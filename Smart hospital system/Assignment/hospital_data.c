...
const char wards[4][30] = {"General Ward", "Paediatric Ward", "Surgical Ward", "ICU (Intensive Care Unit)"};
...
void display_specialties() {
    printf("\n--- Doctor Specialties ---\n");
    for (int i = 0; i < 4; i++) {
        printf("%d. %s - LKR %.2f (Cap: %d/day)\n", i + 1, specialties[i], specialty_fees[i], specialty_caps[i]);
    }
}
...
float get_specialty_fee(int id) {
    if (id >= 1 && id <= 4) return specialty_fees[id - 1];
    return 0.0;
}
