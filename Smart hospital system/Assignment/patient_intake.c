...
void register_patient(Patient patients[], int index) {
    ...
    scanf("%d", &patients[index].is_admitted);

    if (patients[index].is_admitted == 1) {
        display_wards();
...
void display_patient(Patient p) {
    ...
    else if (p.emergency_level == 3) surge_rate = 0.50;

    float surge_charge = doc_fee * surge_rate;
    float ward_cost = (p.is_admitted == 1) ? get_ward_rate(p.ward_id) * p.stay_days : 0.0;
    ...
    printf("Assigned Ward  : %s\n", (p.is_admitted == 1) ? "Ward Admitted" : "OPD / Outpatient");
    printf("Urgency Level  : Level %d (%s)\n", p.emergency_level,
           (p.emergency_level == 3) ? "Critical" : (p.emergency_level == 2) ? "Urgent" : "Normal");
    printf("----------------------------------------------------\n");
...
void generate_reports(Patient patients[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        float surge_rate = (patients[i].emergency_level == 2) ? 0.20 : (patients[i].emergency_level == 3) ? 0.50 : 0.0;
        float gross = doc_fee + (doc_fee * surge_rate) + ((patients[i].is_admitted == 1) ? get_ward_rate(patients[i].ward_id) * patients[i].stay_days : 0.0);
...
void save_to_file(Patient patients[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        fprintf(fp, "ID: PAT-%d | Name: %s | Age: %d | Level: %d | Specialty: %d\n",
                ...
                patients[i].emergency_level, patients[i].specialty_id);
    }
    fclose(fp);
    printf("\n[+] Patient records successfully saved to 'patient_records.txt'!\n");
}
